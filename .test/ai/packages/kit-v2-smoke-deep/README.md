...
```bash
# 从主项目克隆（推荐）
git clone <主项目地址> kit-v2-smoke-deep-eval/

# 或使用 cp -r（无 git 时）
cp -r kit-v2-smoke-deep/ kit-v2-smoke-deep-eval/
```
...
```bash
# 沙盒可随时销毁重建
rm -rf kit-v2-smoke-deep-eval/
```

## 隔离规则

1. **AI 只写沙盒**: AI 代理只能在 `kit-v2-smoke-deep-eval/` 中修改文件，不得直接修改主项目 `kit-v2-smoke-deep/`。
