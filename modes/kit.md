...
进入 brainstorm 前，先确认用户偏好（记录到 `.kit/config.json`）：
...
**Q1. 沟通方式**
[ ] "逐个确认"（推荐初学者）→ 一次一问
[ ] "一口气输出"（有经验用户）→ 完整分析后统一反馈

**Q2. 使用环境（影响路径输出格式）**
[ ] "IDE 环境"（VS Code / Cursor / Windsurf / JetBrains）
    → 路径支持 Ctrl+左键点击打开
    → 输出相对路径即可

[ ] "CLI 环境"（终端 / 纯命令行）
    → 路径需要完整可访问格式
    → 附带查看命令建议（cat / less / code）
...
| 环境 | 路径格式 | 示例 |
|---|---|---|
| IDE | 相对路径（可点击） | `.plan/PRD.md` |
| CLI | 绝对路径 + 查看命令 | `D:\projects\my-app\.plan\PRD.md`（`cat` 或 `code` 打开）|

**检测不到时主动询问：**

如果无法判断用户环境（如用户第一次使用，或 `.kit/config.json` 没有记录）：
...
文档已写入：{cwd}\.plan\{文件名}.md
查看命令：cat {cwd}\.plan\{文件名}.md
...
- 根据环境输出不同格式路径（IDE 可点击 / CLI 带查看命令）
